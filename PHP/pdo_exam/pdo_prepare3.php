...
try {
    ...
    $stmt = $pdo -> prepare("SELECT * FROM girl_group WHERE name >= :nameFrom AND name <= :nameTo");

    // PDOStatement 객체가 가진 쿼리의 파라메터에 변수 값을 바인드
    $stmt -> bindValue(":nameFrom", "소녀시대");
    $stmt -> bindValue(":nameTo", "카라");

    // PDOStatement 객체가 가진 쿼리를 실행
    $stmt -> execute();

    // PDOStatement 객체가 실행한 쿼리의 결과값 가져오기
    $result = $stmt -> fetchAll() ; //몇만개의 데이터값 가져올것 아니면 실행하지말것
