...
//9단계 처음으로 이동하는 알고리즘 
if($group != 1){
					echo"<a href='notice09.php?page=1'>FIRST</a>";
					echo "&nbsp;&nbsp;";
				}
//6단계 이전 5개로 이동하는 루틴.. 즉 이전그룹으로 이동하기 
//prevPage하는 변수는 사용한다
					if($group != 1){
					$prevPage=($group-2)*5+1;
					echo"<a href='notice09.php?page=$prevPage'>PREV</a>";
					echo "&nbsp;&nbsp;";
				}
...
//4단계 그룹변수를 기준으로 startPage와 endPage를 구한다.
					...
					for($i=$startPage;$i<=$endPage;$i++){
							//7단계 해당페이지가 전체 페이지 카운트를 넘기면 break
							if ($i>$pageCount) {break;}
							if($i==$page){
								echo "<a href='notice09.php?page=$i'><font color='red'><b>[$i]</b></font></a> ";	
							}else{
							echo "<a href='notice09.php?page=$i'>[$i]</a> ";
							}
					}
...
//nextPage라는 변수를 사용한다.
						...
						if($group<$groupCount){
						$nextPage=$group*5+1;
						echo "<a href='notice09.php?page=$nextPage'>NEXT</a>";
					}
						echo "&nbsp;&nbsp;";
				//8단계 마지막 페이지로 이동하는 알고리즘	
				if($group<$groupCount){
					$lastPage=$pageCount;
					echo"<a href='notice09.php?page=$lastPage'>LAST</a>";
					echo "&nbsp;&nbsp;";
				}			
				?>
			</td>
