...
<body>
  ...
  <!-- 공지사항영역 시작 -->
  <br><br>
  <table class="notice_Notice">
    <tr>
      <td colspan="3" align="center"><font size="5"><b>공지사항</b></font></td>
      <?php
        $queryNotice=
        "select * 
        from store_notice 
  ...
  <!-- 메뉴정보영역 시작-->
    <table class="notice">
     <tr>
       <td colspan="3"><font size="5"><b>메뉴정보</b></font></td>
     </tr>
     <tr>
      <td><b>음식사진</b></td>
      <td><b>음식명</b></td>
  ...
  <tr>
    ...
    ?>
        <!-- admin일 경우 추가 -->
        <input type="button" class="bt1" value="공지사항추가" onclick="location.href='store_notice.php?storeNo=<?php echo $storeNo; ?>'">
        
      <?php 
      } else if($levelData == "manager"){
      ?>
      <!-- manager일 경우 추가 -->
      <input type="button" class="bt1" value="메뉴추가" onclick="location.href='store_content_insert.php?storeNo=<?php echo $storeNo; ?>&smallType=<?php echo $smallType; ?>&bigTypeNo=<?php echo $bigTypeNo; ?>'">
      <input type="button" class="bt1" value="메뉴삭제" onclick="location.href='store_content_del.php?storeNo=<?php echo $storeNo; ?>&smallType=<?php echo $smallType; ?>&bigTypeNo=<?php echo $bigTypeNo; ?>'">
      <?php 
      }
      ?>  
    <?php
  ...
  <form name="frm1" method="post" action="store_re_ok.php">
    <table class="table_re">
      <tr>
        <th>
          <?php if(isset($_SESSION["id"])) { ?>
          <input type="text" name="memo"size="37">
          <input type="text" name="writer" size="13">&nbsp;
