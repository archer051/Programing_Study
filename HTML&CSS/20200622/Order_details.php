<?php
  ...
  $loginId = $_SESSION["id"];
  $CurDate=date("Ymdhis");
  $BeforeDate = date("Y-m-d h:i:s", strtotime($CurDate." -2 week"));
  $AfterDate = date("Y-m-d h:i:s", strtotime($CurDate."+0 day"));
   $queryOrder=
  "SELECT order_no
		,store_no
		,id
  ...
  FROM store_orders
  WHERE id='$loginId'
  AND order_time <= '$AfterDate'
  AND order_time >= '$BeforeDate'
  order by order_time desc";
  // echo $queryOrder;exit;
  $queryCart1 = "select * from store_orders where id = '$loginId'";
  $rsCart1 = mysqli_query($conn, $queryOrder);
...
?>
<form enctype="multipart/form-data" name="details1" method="post" action="Order_details_ok.php">
<table class="Order">
<!-- <tr>
...
<tr>
  <td>
    <input type="text" name="content" value="<?php echo $content; ?>" readonly>
  </td>
<tr>
<tr>
  <td>
...
<tr>
  <td colspan="2" align="center">
    <input type="button" class="bt1" value="삭제" onclick="submit()">
  </td>
</tr>
</table>
</form>
